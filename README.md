...
本仓库是一个独立的职业能力评估应用，用于把学生的简历、理想岗位/JD 和行为锚定问卷整理成个人界面：

- 始终展示理想岗位能力雷达。
- 问卷支持 10 题快速模式和 48 题详细模式。
- 完成问卷后展示个人能力雷达、合并雷达对比、能力明细和可执行行动清单。
- 未完成问卷时展示“个人雷达还没有测试，暂时无法生成”的空状态。
- 从个人界面补做或重做问卷时，需要先在弹窗中选择快速模式或详细模式。
...
问卷可以立即填写，也可以稍后填写。稍后填写时，后端仍会生成并保存理想岗位雷达；完成问卷后再写回同一个评估会话，生成个人能力雷达。个人界面中的问卷入口同样会弹出模式选择，重新作答会覆盖当前个人能力画像，不保留历史版本。

当前 RAG 的明确使用范围是职业雷达：`POST /assessments/role-profile` 会基于目标岗位/JD 生成岗位能力需求图。个人雷达来自 `POST /assessments/capability-evidence`，基于已保存简历和问卷答案生成能力证据，不是 RAG 检索结果。
...
- `docs/questionnaire-report-improvement-notes.md`：问卷、个人界面、RAG 内容填充和职业个性化 AI 问卷待定事项。
